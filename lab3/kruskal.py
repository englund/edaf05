...
class Kruskal:
    ...
    def find_minimum_spanning_tree(self):
        find_minimum_spanning_tree = set([])
        total_weight = 0
        ...
        for u, v, w in edges_sorted_by_weight:
            if (self.uf.find(u) != self.uf.find(v)):
                self.uf.union(u, v)
                find_minimum_spanning_tree.add((u, v, w))
                total_weight += w
        return find_minimum_spanning_tree, total_weight
...
if __name__ == '__main__':
    ...
    minimum_spanning_tree, total_weight = kruskal.find_minimum_spanning_tree()
    print total_weight
